    ...
    for filename in os.listdir(test_dir):
        if filename.endswith('.json'):
            ...
            try:
                ...
                tests.append({
                    'title': 'Archivo defectuoso',
                    'description': f'Error al cargar el test: {str(e)}',
                    'filename': filename,
                    'error': True,  # marcar como defectuoso
                })
...
def resultado_test(request):
    ...
    context = {
        'tipo_test': 'normal',  # cambiar si es test de inteligencias
        'resultados': resultados,
        'aciertos': aciertos,
        'fallos': fallos,
        'total': total_preguntas,
        'resultado_test_id': resultado_test_db.id  # 👈 ID correcto para exportar PDF
    }
...
@login_required
def exportar_resultado_pdf(request, resultado_id):
    resultado = ResultadoTest.objects.get(id=resultado_id)

    # Renderizar la plantilla
    html = render_to_string('management_test/resultado_pdf.html', {'resultado': resultado})

    # Ruta de wkhtmltopdf en tu sistema
    ...
    # Devolver PDF en la respuesta
    response = HttpResponse(pdf, content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="resultado_{resultado.id}.pdf"'
    return response
